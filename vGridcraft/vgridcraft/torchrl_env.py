...
class GridcraftTorchRLEnv(EnvBase):
    ...
    def _make_specs(self) -> None:
        obs_shape = torch.Size([self.config.num_agents, self.config.obs_size])
        reward_shape = torch.Size([self.config.num_agents, 1])
        action_shape = torch.Size([self.config.num_agents])
        self.observation_spec = Composite(
            agents=Composite(
                observation=Unbounded(shape=obs_shape, dtype=torch.float32, device=self.device),
                shape=torch.Size([self.config.num_agents]),
                device=self.device,
            ),
            shape=torch.Size([]),
            device=self.device,
        ...
        self.done_spec = Composite(
            done=Unbounded(shape=torch.Size([1]), dtype=torch.bool, device=self.device),
            terminated=Unbounded(shape=torch.Size([1]), dtype=torch.bool, device=self.device),
            truncated=Unbounded(shape=torch.Size([1]), dtype=torch.bool, device=self.device),
            shape=torch.Size([]),
            device=self.device,
        )
    ...
    def _reset(self, tensordict=None):
        ...
        return TensorDict(
            {
                ("agents", "observation"): obs["vector"].float(),
