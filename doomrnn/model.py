...
def main():
  ...
  if len(sys.argv) > 4:
    ...
    print("seed", the_seed)
  max_len = int(os.environ.get("WORLD_MODELS_MAX_LEN", "-1"))
  ...
  if final_mode:
    ...
    for i in range(100):
      reward, steps_taken = simulate(model, train_mode=False, render_mode=False, num_episode=1, max_len=max_len)
    ...
    reward, steps_taken = simulate(model,
      train_mode=False, render_mode=render_mode, num_episode=1, max_len=max_len)
